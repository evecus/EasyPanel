...
pub async fn get_processes_handler(
    ...
    let sort = params.get("sort").map(|s| s.clone()).unwrap_or_else(|| "cpu".into());
    let dir = params.get("dir").map(|s| s.clone()).unwrap_or_else(|| "desc".into());
    let limit: usize = params.get("limit").and_then(|v| v.parse().ok()).unwrap_or(100);
    match tokio::task::spawn_blocking(move || get_processes(&sort, &dir, limit)).await.unwrap() {
...
pub async fn kill_process_handler(
    ...
    tokio::task::spawn_blocking(move || kill_process(pid)).await.unwrap()
...
pub async fn container_action_handler(
    ...
    tokio::task::spawn_blocking(move || container_action(&id, &action)).await.unwrap()
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": e.to_string()}))))?;
...
pub async fn get_container_logs_handler(
    ...
    let lines: usize = params.get("lines").and_then(|v| v.parse().ok()).unwrap_or(200);
    tokio::task::spawn_blocking(move || get_container_logs(&id, lines)).await.unwrap()
...
pub async fn inspect_container_handler(
    ...
    tokio::task::spawn_blocking(move || inspect_container(&id)).await.unwrap()
...
pub async fn get_compose_file_handler(
    ...
    let path = params.get("path").cloned()
        .ok_or_else(|| (StatusCode::BAD_REQUEST, Json(serde_json::json!({"error":"path required"}))))?;
    tokio::task::spawn_blocking(move || read_compose_file(&path)).await.unwrap()
        .map(|content| Json(serde_json::json!({"content": content})))
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": e.to_string()}))))
}

#[derive(Deserialize)]
pub struct ApplyComposeReq { pub path: String, pub content: String, pub container_id: String }

pub async fn apply_compose_handler(
    Json(body): Json<ApplyComposeReq>,
) -> Result<Json<serde_json::Value>, (StatusCode, Json<serde_json::Value>)> {
    let (path, content, cid) = (body.path.clone(), body.content.clone(), body.container_id.clone());
    let log = tokio::task::spawn_blocking(move || write_and_apply_compose(&path, &content, &cid)).await.unwrap()
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": e.to_string()}))))?;
    invalidate_docker_cache();
...
pub async fn pull_update_container_handler(
    ...
    let log = tokio::task::spawn_blocking(move || pull_and_update_container(&id)).await.unwrap()
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": e.to_string()}))))?;
    invalidate_docker_cache();
...
pub async fn service_action_handler(
    ...
    tokio::task::spawn_blocking(move || service_action(&unit, &action)).await.unwrap()
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": e.to_string()}))))?;
...
pub async fn get_service_logs_handler(
    ...
    let lines: usize = params.get("lines").and_then(|v| v.parse().ok()).unwrap_or(200);
    tokio::task::spawn_blocking(move || get_service_logs(&unit, lines)).await.unwrap()
        .map(|logs| Json(serde_json::json!({"logs": logs})))
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": e.to_string()}))))
}
