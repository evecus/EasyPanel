...
pub async fn login(
    ...
    let user = config::find_user(&req.username)
    ...
    let token = make_token(&user.username)
        .map_err(|_| (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error":"token error"}))))?;
    ...
    Ok((jar.add(cookie), Json(serde_json::json!({
        "token": token,
...
pub async fn get_me(
    ...
    Ok(Json(serde_json::json!({"username": user.username, "nickname": user.nickname, "is_admin": user.is_admin})))
}

pub async fn check_auth(jar: CookieJar) -> Json<serde_json::Value> {
    let token_str = jar.get("token").map(|c| c.value().to_string());
    let Some(token) = token_str else { return Json(serde_json::json!({"logged_in": false})); };
    let Some(username) = verify_token(&token) else { return Json(serde_json::json!({"logged_in": false})); };
    let Some(user) = config::find_user(&username) else { return Json(serde_json::json!({"logged_in": false})); };
    Json(serde_json::json!({"logged_in": true, "username": user.username, "nickname": user.nickname, "is_admin": user.is_admin}))
}

#[derive(Deserialize)]
pub struct UpdatePasswordReq { pub old_password: String, pub new_password: String }
...
pub async fn update_password(
    ...
    if !verify(&req.old_password, &user.password).unwrap_or(false) {
        return Err((StatusCode::UNAUTHORIZED, Json(serde_json::json!({"error":"wrong password"}))));
    }
    ...
    config::with_main_mut(|cfg| {
        if let Some(u) = cfg.users.iter_mut().find(|u| u.username == username) { u.password = new_hash; }
    }).map_err(|_| (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error":"save failed"}))))?;
    Ok(Json(serde_json::json!({"ok": true})))
}

#[derive(Deserialize)]
pub struct UpdateNicknameReq { pub nickname: String }
...
pub async fn update_nickname(
    ...
    let username = state.current_user.clone().unwrap_or_default();
    config::with_main_mut(|cfg| {
        if let Some(u) = cfg.users.iter_mut().find(|u| u.username == username) { u.nickname = req.nickname.clone(); }
    }).map_err(|_| (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error":"save failed"}))))?;
    Ok(Json(serde_json::json!({"ok": true})))
}
...
#[derive(Deserialize)]
pub struct CreateUserReq { pub username: String, pub password: String, pub nickname: String, pub is_admin: bool }

pub async fn create_user(
    Json(req): Json<CreateUserReq>,
) -> Result<Json<serde_json::Value>, (StatusCode, Json<serde_json::Value>)> {
    if config::find_user(&req.username).is_some() {
        return Err((StatusCode::CONFLICT, Json(serde_json::json!({"error":"user exists"}))));
    }
    let hash = bcrypt::hash(&req.password, bcrypt::DEFAULT_COST)
        .map_err(|_| (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error":"hash error"}))))?;
    config::with_main_mut(|cfg| {
        cfg.users.push(config::User { username: req.username.clone(), password: hash, nickname: req.nickname.clone(), is_admin: req.is_admin });
    }).map_err(|_| (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error":"save failed"}))))?;
    Ok(Json(serde_json::json!({"ok": true})))
}
...
pub async fn delete_user(
    ...
    if target == me {
        return Err((StatusCode::BAD_REQUEST, Json(serde_json::json!({"error":"cannot delete self"}))));
    }
    let mut found = false;
    config::with_main_mut(|cfg| {
        if let Some(pos) = cfg.users.iter().position(|u| u.username == target) { cfg.users.remove(pos); found = true; }
    }).map_err(|_| (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error":"save failed"}))))?;
    if found { Ok(Json(serde_json::json!({"ok": true}))) }
    else { Err((StatusCode::NOT_FOUND, Json(serde_json::json!({"error":"not found"})))) }
}
