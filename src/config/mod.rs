...
pub fn data_dir() -> String {
    DATA_DIR_INNER.read().unwrap().clone().unwrap_or_else(|| "data".into())
}
pub fn config_dir() -> String {
    CONFIG_DIR_INNER.read().unwrap().clone().unwrap_or_else(|| "data/config".into())
}
pub fn config_path() -> String {
    CONFIG_PATH_INNER.read().unwrap().clone().unwrap_or_else(|| "data/config/rspanel.yaml".into())
}

// Keep these as pub aliases so handler code that still imports them compiles
pub fn DATA_DIR() -> String { data_dir() }
